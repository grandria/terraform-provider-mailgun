...
* `web_prefix` - (Optional) The prefix used for tracking domain, click and unsubscribe URLs. Default: `email`. **Note**: Updating this value requires updating your domain's DNS to include the corresponding CNAME record (e.g., `<web_prefix>.<your-domain.com>`).
